...
describe("User Controller Integration", () => {
  ...
  describe("POST /api/v1/user/:id/follow", () => {
    it("follows another user and returns message", async () => {
      const res = await request(app).post(`/api/v1/user/${otherUser._id}/follow`).set("Authorization", `Bearer ${token}`).expect(200);

      expect(res.body.message).to.match(/You have followed/);
    });

    it("rejects following yourself", async () => {
      await request(app).post(`/api/v1/user/${user._id}/follow`).set("Authorization", `Bearer ${token}`).expect(403);
    });

    it("rejects invalid userId param", async () => {
      await request(app).post("/api/v1/user/invalidId/follow").set("Authorization", `Bearer ${token}`).expect(400);
    });
  });

  describe("GET /api/v1/user/:id/posts", () => {
    it("returns user posts paginated", async () => {
      const res = await request(app).get(`/api/v1/user/${user._id}/posts?page=1`).set("Authorization", `Bearer ${token}`).expect(200);
    ...
    it("404 on nonexistent user", async () => {
      await request(app).get(`/api/v1/user/${new mongoose.Types.ObjectId()}/posts`).set("Authorization", `Bearer ${token}`).expect(404);
    });
  });
  ...
  describe("PATCH /api/v1/user/updateMe", () => {
    ...
    it("rejects password field in body", async () => {
      await request(app).patch("/api/v1/user/updateMe").set("Authorization", `Bearer ${token}`).send({ password: "x" }).expect(400);
    });
  });
